...
const Header = () => {
  ...
  return (
    <header className="flex justify-between px-8 py-2">
      ...
      {!connect &&<button onClick={handleSelectWallet} className="border border-[#13172a] p-1 rounded-md font-semibold">Connect wallet</button>}
      {connect && <p className="border border-[#13172a] p-1 rounded-md font-semibold flex items-center">{wallet !== '' && cropWallet(wallet)}</p>}
    </header>
  );
};
